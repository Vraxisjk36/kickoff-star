...
ok(SAVE_SCHEMA_VERSION>=7&&'octoberLeague'in EMPTY_CUPS&&'youthFestival'in EMPTY_CUPS&&'academyChampionsCup'in EMPTY_CUPS,'new competitions persist through V4 save schema')
...
ok(saveSrc.includes("raw.player?.sundayLeague && raw.player?.schoolId ? 'pending'")&&loadSrc.includes("schoolDevelopment:null")&&loadSrc.includes("youthFestival:null"),'all legacy dual-route saves are flagged and abandoned route state is pruned')
