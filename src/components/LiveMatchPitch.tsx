...
import { PITCH_POSITION_SLOT, pitchShirtNumbers } from '../engine/matchPresentation'
...
function shirtInk(color:string){
 const hex=color.replace('#','')
 if(!/^[0-9a-f]{6}$/i.test(hex)) return '#fff'
 const [r,g,b]=[0,2,4].map(i=>parseInt(hex.slice(i,i+2),16))
 return (r*.299+g*.587+b*.114)>145?'#141414':'#fff'
}
export type PitchAction='idle'|'attack'|'shot'|'save'|'goal'|'tackle'|'cross'
export default function LiveMatchPitch({momentum,homeColor,awayColor,homeShort,awayShort,playerIsHome,player,playerOnPitch,minute,action='idle',focusPlayer=false,scoringSide}:{momentum:number;homeColor:string;awayColor:string;homeShort:string;awayShort:string;playerIsHome:boolean;player:Player;playerOnPitch:boolean;minute:number;action?:PitchAction;focusPlayer?:boolean;scoringSide?:'home'|'away'}){
 const userIndex=PITCH_POSITION_SLOT[player.position]
 const userNumbers=pitchShirtNumbers(player,playerOnPitch)
 const opponentNumbers=[1,2,4,5,3,6,8,10,7,9,11]
 ...
 return <div className={'live-pitch relative w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl'+cameraClass+(focusPlayer?' player-focus':'')} style={{aspectRatio:'3/2'}}>
  ...
  {SHAPE.map((p,i)=><div key={'a'+i} className={'match-shirt '+(playerOnPitch&&playerIsHome&&i===userIndex?'user-player':'')} title={`${homeShort} #${(playerIsHome?userNumbers:opponentNumbers)[i]}`} style={{left:`${p.x}%`,top:`${clamp(p.y+shift,5,94)}%`,backgroundColor:homeColor,color:shirtInk(homeColor),animationDelay:`${i*.07}s`}}><span>{(playerIsHome?userNumbers:opponentNumbers)[i]}</span></div>)}
  {SHAPE.map((p,i)=><div key={'b'+i} className={'match-shirt '+(playerOnPitch&&!playerIsHome&&i===userIndex?'user-player':'')} title={`${awayShort} #${(!playerIsHome?userNumbers:opponentNumbers)[i]}`} style={{left:`${100-p.x}%`,top:`${clamp(100-p.y-shift,5,94)}%`,backgroundColor:awayColor,color:shirtInk(awayColor),animationDelay:`${i*.07+.2}s`}}><span>{(!playerIsHome?userNumbers:opponentNumbers)[i]}</span></div>)}
  ...
  <div className="absolute left-3 bottom-2 text-[8px] uppercase tracking-[.18em] text-white/70">{homeShort} · {awayShort}</div>
 </div>
}
