...
export function matchAvailability(player: Player) {
  ...
  return {canPlay:energy>=30,canStart:energy>=50,reason:energy<30?'Below 30% energy: you sit this match out.':energy<50?'Below 50% energy: you can only play from the bench.':''}
}
export function playerForMatch(player: Player,_competitionId:string): Player {
  const availability=matchAvailability(player)
  const squadRole=!availability.canPlay?'reserves':!availability.canStart&&player.squadRole==='starting-xi'?'bench':player.squadRole
  return {...player,squadRole,matchEnergyMultiplier:1}
}
