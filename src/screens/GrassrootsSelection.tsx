...
export default function GrassrootsSelection({ onChoose }: { onChoose: (club: GrassrootsClubChoice) => void }) {
 ...
 return <div className="relative min-h-screen w-full bg-ks-black px-5 py-8"><div className="relative z-10 max-w-md mx-auto flex min-h-[85vh] flex-col">
  ...
  <div className="flex flex-col gap-3 flex-1">{GRASSROOTS_STARTER_CLUBS.map(club=><button key={club.id} onClick={()=>setSelected(club)} className={`text-left rounded-2xl border px-5 py-4 ${selected?.id===club.id?'border-ks-gold bg-ks-gold/10':'border-ks-border bg-[#0f0f0d]'}`}><div className="font-display text-ks-ink text-lg mb-1">{club.name}</div><p className="text-[12px] text-ks-muted mb-3">{club.blurb}</p><div className="text-[10px] text-ks-gold">{club.strengths.join(' · ')}</div></button>)}</div>
