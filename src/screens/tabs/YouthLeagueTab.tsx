...
export default function YouthLeagueTab({youthWorld,initialView='fixtures'}:{youthWorld:YouthWorld;initialView?:'fixtures'|'table'}){
 ...
 const recent=fixtures.filter(f=>f.played).sort((a,b)=>b.round-a.round).slice(0,5)
 const upcoming=fixtures.filter(f=>!f.played).sort((a,b)=>a.round-b.round).slice(0,6)
 ...
 return <div className="flex flex-col gap-2.5">
  ...
  {view==='table'?<div className="rounded-lg border border-ks-border bg-[#0f0f0d] overflow-hidden"><div className="grid grid-cols-[1.75rem_1fr_2rem_2rem_2rem] gap-1 px-3 py-2 text-[9px] text-ks-muted uppercase"><span>#</span><span>team</span><span>P</span><span>GD</span><span>PTS</span></div>{table.map((s,i)=><div key={s.teamId} className={`grid grid-cols-[1.75rem_1fr_2rem_2rem_2rem] gap-1 px-3 py-2 text-[11px] border-t border-ks-border/30 ${s.teamId===teamId?'bg-ks-gold/10 text-ks-gold':'text-ks-ink'}`}><span>{i+1}</span><span className="truncate">{names.get(s.teamId)}</span><span>{s.played}</span><span>{s.goalDifference>0?'+':''}{s.goalDifference}</span><b>{s.points}</b></div>)}</div>:view==='stats'?<div className="space-y-2">{statPlayers.length?[["TOP SCORERS",goals,"goals","G"],["TOP ASSISTS",assists,"assists","A"],["BEST RATINGS",ratings,"rating",""],["CLEAN SHEETS",cleanSheets,"cleanSheets","CS"]].map(([title,list,metric,suffix])=><div key={title as string} className="rounded-lg border border-ks-border bg-[#0f0f0d] p-3"><div className="font-display text-[10px] tracking-widest text-ks-gold mb-2">{title as string}</div>{(list as typeof statPlayers).map((p,i)=><div key={p.playerId} className="grid grid-cols-[1.2rem_1fr_3rem] py-1 text-[10px]"><span className="text-ks-muted">{i+1}</span><span className="text-ks-ink truncate">{p.name}</span><b className="text-ks-gold text-right">{metric==='rating'?avg(p).toFixed(2):`${p[metric as 'goals'|'assists'|'cleanSheets']}${suffix}`}</b></div>)}</div>):<div className="rounded-lg border border-ks-border bg-[#0f0f0d] p-4 text-[11px] text-ks-muted">Competition statistics populate from played V5 fixtures.</div>}</div>:<div className="space-y-2">
   <div className="rounded-lg border border-ks-border bg-[#0f0f0d] overflow-hidden"><div className="px-3 py-2 text-[9px] tracking-[.2em] text-ks-gold font-display">RECENT RESULTS</div>{recent.length?recent.map((f:CompetitionFixture)=><div key={f.id} className="px-3 py-3 border-t border-ks-border/30 text-xs flex gap-2"><span className="text-ks-muted w-7">R{f.round}</span><span className="flex-1 text-ks-ink">{names.get(f.homeTeamId)} <b className="text-ks-gold">${f.homeGoals}–${f.awayGoals}</b> {names.get(f.awayTeamId)}</span></div>):<div className="px-3 py-4 border-t border-ks-border/30 text-[11px] text-ks-muted">No league results yet.</div>}</div>
   <div className="rounded-lg border border-ks-border bg-[#0f0f0d] overflow-hidden"><div className="px-3 py-2 text-[9px] tracking-[.2em] text-ks-gold font-display">UPCOMING FIXTURES</div>{upcoming.length?upcoming.map((f:CompetitionFixture)=><div key={f.id} className="px-3 py-3 border-t border-ks-border/30 text-xs flex gap-2"><span className="text-ks-muted w-7">R{f.round}</span><span className="flex-1 text-ks-ink">{names.get(f.homeTeamId)} <b className="text-ks-muted">vs</b> {names.get(f.awayTeamId)}</span></div>):<div className="px-3 py-4 border-t border-ks-border/30 text-[11px] text-ks-muted">No fixtures remaining.</div>}</div>
  </div>}
 </div>
}
