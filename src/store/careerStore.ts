...
import { worldSchool,localSchoolChoices } from '../engine/youthWorldDatabase'
...
export const useCareerStore = create<CareerStore>((setState, getState) => ({
  ...
  ensureLeagueWorld: () => {
    ...
    const school = player.schoolId ? (worldSchool(player.schoolId,player.nationality??'eng') ?? getSchool(player.schoolId)) : undefined
  ...
  advanceToNextWeek: () => {
    ...
    if (!isInAcademy && updatedLeague?.kind === 'school' && !result.seasonEnded) {
      ...
      if(completedWeekNumber===23){const d=updatedLeague.divisions[updatedLeague.playerDivision];const position=sortStandings(d.standings).findIndex(s=>s.teamId===updatedLeague!.playerTeamId)+1;const qualified=position>0&&position<=3;if(!qualified){const base=d.teams.find(t=>t.id===updatedLeague!.playerTeamId);updatedCups={...updatedCups,schoolCup:updatedCups.schoolCup?{...updatedCups.schoolCup,playerEliminated:true}:null,schoolDevelopment:base?initCupById('schoolDevelopment',base,d.teams):updatedCups.schoolDevelopment}};if(qualified)updatedPlayer={...updatedPlayer,competitionCareer:addQualification(updatedPlayer.competitionCareer,{competitionId:'schoolLeague',qualifiedFor:'schoolCup',season:calendar.currentWeek.seasonYear,reason:'league-position',position})};updatedPlayer=withStory(updatedPlayer,result.calendar,{kind:qualified?'qualification':'elimination',eyebrow:'Local School League · final table',title:qualified?'REGIONAL CUP QUALIFIED':'DEVELOPMENT COMPETITION',body:qualified?`A ${position}${position===1?'st':position===2?'nd':'rd'}-place finish sends your school into the 24-team Regional Schools Cup.`:'Your school missed the top three, so the next five fixtures move into the Schools Development Competition. Your individual performances still count toward Regional XI selection.',detail:qualified?'Four groups of six. Five group matches. The top two in each group reach the quarter-finals.':'Selection staff judge the player, not only the school result.',ceremony:qualified?'draw':'selection'})}
  ...
  resolveAcademyTrial:(offerId,passed,score)=>{const{player,calendar}=getState();if(!player)return;const offer=player.contractOffers.find(x=>x.id===offerId);if(!offer||!academyTrialAvailable(player,offerId,calendar))return;passed=passed&&score>=62;let updated:Player={...player,contractOffers:passed?player.contractOffers:player.contractOffers.filter(x=>x.id!==offerId),pathway:{...(player.pathway??initYouthPathway(player)),academyTrialStatus:passed?'passed':'failed',academyTrialClubId:offer.clubId},competitionCareer:recordSelectionResult(player.competitionCareer,{competitionId:'academyTrials',season:calendar?.currentWeek.seasonYear??1,round:1,entrants:30,survivors:16,score,outcome:passed?'selected':'cut'})};
if(passed){
  const candidates=player.scoutWatchers.filter(w=>w.clubId!==offer.clubId).sort((a,b)=>b.interest-a.interest)
  const desired=Math.min(6,Math.max(3,1+Math.floor(score/20)))
  const additions=candidates.slice(0,desired-1).map(w=>({id:crypto.randomUUID(),clubId:w.clubId,clubName:w.clubName,clubShort:w.clubShort,ratings:w.ratings,prestige:w.prestige,kind:'academy' as const,weekOffered:player.totalWeeksElapsed??0,expiresInWeeks:2}))
  updated={...updated,contractOffers:[offer,...additions]}
}
updated=withStory(updated,calendar,{kind:passed?'selection':'elimination',eyebrow:`${offer.clubName} academy trial`,title:passed?'TRIAL PASSED':'TRIAL ENDS HERE',body:passed?`${updated.contractOffers.filter(o=>o.kind==='academy').length} academies are ready to discuss a scholarship.`:'The academy chose other players, but you return to your current team with the pathway still alive.',detail:`Trial score ${score}/100 · required 62.`,ceremony:'selection',metrics:[{label:'Trial score',value:score,suffix:'/100',tone:passed?'good':'bad'},{label:'Pass mark',value:62,suffix:'/100'}]});setState({player:updated});void getState().saveCurrent()},
